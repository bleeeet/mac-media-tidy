...
export async function processImage(item, { trash, imageCompress = true }) {
  const base = {
  ...
  try {
    ...
    if (keepPng) {
      ...
      // HEIC 转 JPG 常常反而超过 3MB，此时不能只编一档就收工：产物超标就继续降档。
      // 每一档都从原始文件重新编码，避免有损叠加。
      let dim = decision.compress ? maxDim : null;
      for (const quality of qualityLadder()) {
        outSize = await encodeJpeg(item.path, tmpJpg, quality, dim);
        if (outSize <= MAX_IMAGE_BYTES) break;
        if (!imageCompress) break;
        dim = maxDim;
      }
      outPath = tmpJpg;
    }
