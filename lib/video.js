...
import { parseFps, targetSize, needsCompress, FPS_LIMIT, MIN_VIDEO_BYTES } from './video-rules.js';
...
export async function processVideo(item, { trash, encoder, onProgress }) {
  ...
  // 体积够小就直接放过，连 ffprobe 都不必跑
  if (item.size < MIN_VIDEO_BYTES) {
    return { ...base, action: 'skipped', note: '小于 10MB' };
  }
